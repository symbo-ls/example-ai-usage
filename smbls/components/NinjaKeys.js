...
// Helper to add filter (combines with existing)
const addFilter = (rootState, newFilter) => {
  const current = rootState?.filter || {}
  rootState?.update({ filter: { ...current, ...newFilter } })
}

// Helper to remove a specific filter key
const removeFilter = (rootState, key) => {
  const current = rootState?.filter || {}
  const updated = { ...current }
  delete updated[key]
  rootState?.update({ filter: Object.keys(updated).length ? updated : null })
}

const buildNinjaData = (fleet, rootState, el) => {
  ...
  const data = [
    ...
    },
    {
      ...
      handler: () => { rootState?.update({ modal: '/add-network' }) }
    }
  ]
  ...
  data.push({
    ...
    title: currentFilter.type === 'validator' ? '✓ Validators (click to remove)' : 'Add filter: Validators',
    section: 'Filters',
    ...
    handler: () => {
      if (currentFilter.type === 'validator') {
        removeFilter(rootState, 'type')
      } else {
        addFilter(rootState, { type: 'validator' })
      }
    }
  })
  ...
  data.push({
    id: 'filter-rpc',
    title: currentFilter.type === 'rpc' ? '✓ RPC Nodes (click to remove)' : 'Add filter: RPC Nodes',
    section: 'Filters',
  ...
  envs.forEach(env => {
    ...
    data.push({
      ...
      handler: () => {
        if (isActive) {
          removeFilter(rootState, 'env')
        } else {
          addFilter(rootState, { env })
        }
      }
    })
  })
  ...
  clouds.forEach(cloud => {
    ...
    data.push({
      ...
      title: isActive ? `✓ ${cloud} (click to remove)` : `Add filter: ${cloud}`,
      ...
      handler: () => {
        if (isActive) {
          removeFilter(rootState, 'cloud')
        } else {
          addFilter(rootState, { cloud })
        }
      }
    })
  })
  ...
  rewardClaims.forEach(claim => {
    ...
    data.push({
      ...
      title: isActive ? `✓ ${claim} (click to remove)` : `Add filter: ${claim} Rewards`,
      ...
      handler: () => {
        if (isActive) {
          removeFilter(rootState, 'rewardClaim')
        } else {
          addFilter(rootState, { rewardClaim: claim })
        }
      }
    })
  })
  ...
  fleet.forEach(network => {
    const iconSrc = getNetworkIcon(network.protocol)

    data.push({
      id: `network-${network.protocol}`,
      title: network.protocol,
      section: 'Networks',
      icon: createImgIcon(iconSrc) || '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle></svg>',
      keywords: `${network.network_type || ''} ${network.network_layer || ''}`,
      handler: () => { navigate(`/network/${network.protocol}`) }
    })
    ...
    network.validators?.forEach(validator => {
      data.push({
        ...
        section: 'Validators',
        icon: createImgIcon(iconSrc) || '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#4ade80" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>',
        ...
        handler: () => { navigate(`/node/${network.protocol}/validator/${validator.uid}`) }
      })
    })
    ...
    network.rpc_nodes?.forEach(rpc => {
      data.push({
        ...
        section: 'RPC Nodes',
        icon: createImgIcon(iconSrc) || '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg>',
        ...
        handler: () => { navigate(`/node/${network.protocol}/rpc/${rpc.uid}`) }
      })
    })
  })
  ...
  props: {
    ...
    onRender: async (el, s) => {
      ...
      if (!window.__ninjaKeys) {
        ...
        ninja.setAttribute('placeholder', 'Search... (combine filters: env:testnet cloud:gcp)')
      ...
      const updateData = () => {
        ...
        ninja.data = buildNinjaData(fleet, s.root, el)
      }
