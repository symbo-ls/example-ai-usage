export const NetworkRow = {
    ...
    parseNetworkRow: function parseNetworkRow(data) {
      ...
      if (Array.isArray(data.validators) && data.validators.length > 0) {
        ...
        data.validators.forEach((validator) => {
          if (validator.cloud_provider)
            result.cloud_provider.add(validator.cloud_provider);
      ...
      if (Array.isArray(data.rpc_nodes) && data.rpc_nodes.length > 0) {
        ...
        data.rpc_nodes.forEach((node) => {
          if (node.cloud_provider)
            result.cloud_provider.add(node.cloud_provider);
  ...
  props: {
    ...
    onInit: (el, s) => {
      const parsed = el.scope.parseNetworkRow(s);
      s.parsed = parsed;
    },
  },
  Name: {
    AvatarBox: {
      ...
      onClick: (ev, el, s) => {
        ...
        box.style.cssText =
          "background:#1a1a1a;padding:24px;border-radius:12px;min-width:320px";
        box.innerHTML = `
